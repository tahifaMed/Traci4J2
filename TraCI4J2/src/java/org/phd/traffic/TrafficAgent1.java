...
public class TrafficAgent1 extends Agent {
		...
		public void action() {
			...
			if (msg != null) {
				try {
					...
					learningAlgorithm = new LearningAlgorithm(junction);
	...
	class learnBehavior extends Behaviour {
		...
		@Override
		public void action() {
			MessageTemplate mt1 = MessageTemplate.MatchContent("stop");
			...
			if (messagestop != null) {
				...
				MessageTemplate mt = MessageTemplate.MatchContent("learn");
				...
				try {
					...
					while (msg != null) {
						System.out.println("here TrafficAgent");
						synchronized (Junction.class) {
							//junction.calculateNumberVehiclesInDetectors();
							System.out.println("detectors");
						}
					}

					//junction.printNumberVehiclesInDetectors();
					...
					Data data = new Data(numVehicle, connection
							.getTraciconnection().getCurrentSimStep());
					ACLMessage message = new ACLMessage(ACLMessage.INFORM);
